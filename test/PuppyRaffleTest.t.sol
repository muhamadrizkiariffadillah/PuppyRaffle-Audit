...
contract PuppyRaffleTest is Test {
    ...
    function testDenialOfService() public {
        ...
        for(uint256 idx = 0; idx < playersNum; idx++){
            players[idx] = address(idx);
        }
        ...
        for(uint256 idx = 0; idx < playersNum; idx++){
            playersSecond[idx] = address(idx + playersNum);
        }

        uint gasStartSecond = gasleft();
        puppyRaffle.enterRaffle{value: entranceFee * players.length}(playersSecond);
        uint gasEndSencond = gasleft();
        uint gasUsedSecond = (gasStartSecond - gasEndSencond) * tx.gasprice;
